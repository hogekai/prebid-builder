...
const allRecommendedModules = [...basicRecommendedModules, "schain"];
...
async function resolveModules(options: BuildOptions): Promise<string[]> {
  ...
  if (options.recommended === "basic") {
    basicRecommendedModules.forEach((module) => modulesSet.add(module));
  } else if (options.recommended === "all") {
    allRecommendedModules.forEach((module) => modulesSet.add(module));
  }
...
async function copyPrebidOutput(output: string): Promise<void> {
  const sourcePath = path.resolve(resolvePrebidPath(), "build/dist/prebid.js");
